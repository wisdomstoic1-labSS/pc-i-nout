...
Кадр сгенерирован через vidIQ по промпту из §3, подпись наложена скриптом.

| Файл | Что |
|---|---|
| `thumbnails/warsaw-aerial-src-1300.png` | исходный кадр без подписей |
| `thumbnails/warsaw-aerial-2075.png` | **вариант B: `1300 ⟶ 2075`** |
| `thumbnails/warsaw-aerial-2026.png` | вариант A: `1300 ⟶ 2026` |

```bash
python3 tools/miniatura_aerial.py \
  thumbnails/warsaw-aerial-src-1300.png \
  thumbnails/warsaw-aerial-2075.png WARSAW 1300 2075
```

Кадр лёг под раскладку без правок: горизонт на 22% высоты, верхняя полоса неба и
нижний луг спокойные, городок правее центра, излучина левее. Требование про
спокойные края из §3 сработало — двигать обрезку не пришлось.

**Про оценку vidIQ.** Скорер выставил 30 из 100 с советом «добавьте стрелку,
чтобы направить взгляд». Стрелка — ровно то, что накладывается следующим шагом,
скорер её просто не видел. Он же требует сплит-скрин, хотя весь смысл этого
формата в том, что второй половины нет.
